...
*/
    if(MO>0) // AP
    {
        ...
        if(e[OBJ_DETECT]&&(obj2follow==ANY_OBJ)) act(REACH_OBJ,0.1);
        // if the robot should follow only the obj zero, and evade the rest
        if((e[OBJ_DETECT])&&obj2follow==ALL_BUT_ZERO)
        {
            if(e[OBJ_DETECT]&1) act(REACH_OBJ,8.0);
            else act(EVADE_OBJ,1.0);
        }
        // when the detected object is the right one.
        if(e[OBJ_DETECT]&&(obj2follow>=0))
        {
    ...
    else
    {
        ...
        if(e[OBJ_DETECT]&&(obj2follow==ANY_OBJ)) act(EVADE_OBJ,1.0);
        // if the robot should follow only the obj zero, and evade the rest
        if(e[OBJ_DETECT]&&(obj2follow==ALL_BUT_ZERO))
        {
            if(e[OBJ_DETECT]&1) act(REACH_OBJ,8.0);
