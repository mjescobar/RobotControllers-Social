...
void behexec::f_modi_follow()
{
    ...
    case 2:
        ...
        if(modi2detect==ANY_MODI)
        {
            if(ENABLE_FORAGING)
            {
                /* the first criterion is searching for
                   the closest and positive robot */
                for(int i=0; i< MAX_OBJECTS; i++)
                    if((rob->m_pos[i].m>=0)&&(rob->mo_sur[i]>=0)&&i!=rob->id)
                    {
                ...
                /* if the previous criterion is not met by any robot,
                   the positive condition is eliminated */
                if(modi2detect==ANY_MODI)
                {
                    for(int i=0; i< MAX_OBJECTS; i++)
